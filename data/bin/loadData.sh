...
loadData() {
    ...
    if [ -s ${CSV_FILE} ]; then
        sfdx force:data:bulk:upsert -u ${CONNECTION_ALIAS} -f ${CSV_FILE} -s ${SOBJECT_TYPE} -i ${EXTERNAL_ID_FIELD} -w 5
        if [ $? -ne 0 ]; then
...
createData() {
    ...
    if [ -s ${APEX_FILE} ]; then
        sfdx force:apex:execute -u ${CONNECTION_ALIAS} -f ${APEX_FILE}
        if [ $? -ne 0 ]; then
...
if [ $# -eq 0 ]; then
    echo "Usage: loadData.sh <sPRS-namespace> [<connectionAlias>]"
    exit 1
fi
...
CONNECTION_ALIAS=sirono-salesforce
if [ $# -eq 2 ]; then
    CONNECTION_ALIAS=$2
fi
echo "Loading data for connection alias ${CONNECTION_ALIAS}."
...
sfdx force:org:display -u ${CONNECTION_ALIAS} 2> /dev/null
if [ $? -ne 0 ]; then
    echo "Please log into the target org using sfdx ... (this is annoying, I know)."
    sleep 1s
    sfdx force:auth:web:login -a ${CONNECTION_ALIAS}
    if [ $? -ne 0 ]; then
        echo "Login failed."
        exit 1
    fi
...
sfdx force:mdapi:deploy -d ./res/documents/deploy -u ${CONNECTION_ALIAS} -w 5

# Make sure the static resource deploys because if it doesn't, no use creating attachments later on.
if [ $? -ne 0 ]; then
...
sfdx force:mdapi:deploy -d ./res/documents/undeploy -u ${CONNECTION_ALIAS} -w 5
